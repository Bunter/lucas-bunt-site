...
export async function POST(request:Request){
  try{
    ...
    if(action==="delete"||action==="unpublish"){
      ...
      if(action==="publish"&&content.category==="photography"&&!content.assets.some(a=>a.mime.startsWith("image/")))return Response.json({error:"Add a photo before publishing."},{status:400});
