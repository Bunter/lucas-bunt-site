...
export function EntryBody({content}:{content:EntryContent}){return <article className="entry-body">
  ...
  {content.category==="restaurant-reviews"&&<RestaurantDetails restaurant={content.restaurant}/>}
  ...
  {content.category==="cooking-recipes"&&!!content.recipeTags?.length&&<footer className="recipe-tags"><h2>Dietary tags</h2><ul aria-label="Dietary tags">{content.recipeTags.map(tag=><li key={tag}>{tag}</li>)}</ul></footer>}
</article>;}
