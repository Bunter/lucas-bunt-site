...
export async function publicEntry(id:string){ const r=await database().prepare("SELECT published FROM entries WHERE id=? AND published IS NOT NULL").bind(id).first<{published:string}>();return r?JSON.parse(r.published) as EntryContent:null; }
